...
import com.mickeymeng.framework.middleware.job.core.util.CronUtils;
...
public class JobServiceImpl implements JobService {
    ...
    public Long createJob(JobCreateReqVO createReqVO) throws SchedulerException {
        ...
        if (jobMapper.selectByHandlerName(createReqVO.getHandlerName()) != null) {
            throw exception(ErrorCodeConstants.JOB_HANDLER_EXISTS);
        }
        // 插入
    ...
    public void updateJob(JobUpdateReqVO updateReqVO) throws SchedulerException {
        ...
        if (!job.getStatus().equals(JobStatusEnum.NORMAL.getStatus())) {
            throw exception(ErrorCodeConstants.JOB_UPDATE_ONLY_NORMAL_STATUS);
        }
        // 更新
    ...
    public void updateJobStatus(Long id, Integer status) throws SchedulerException {
        // 校验 status
        if (!CollectionUtils.containsAny(status, JobStatusEnum.NORMAL.getStatus(), JobStatusEnum.STOP.getStatus())) {
            throw exception(ErrorCodeConstants.JOB_CHANGE_STATUS_INVALID);
        }
        // 校验存在
        JobDO job = this.validateJobExists(id);
        // 校验是否已经为当前状态
        if (job.getStatus().equals(status)) {
            throw exception(ErrorCodeConstants.JOB_CHANGE_STATUS_EQUALS);
        }
        // 更新 Job 状态
    ...
    private JobDO validateJobExists(Long id) {
        ...
        if (job == null) {
            throw exception(ErrorCodeConstants.JOB_NOT_EXISTS);
        }
        return job;
    }
    ...
    private void validateCronExpression(String cronExpression) {
        if (!CronUtils.isValid(cronExpression)) {
            throw exception(ErrorCodeConstants.JOB_CRON_EXPRESSION_VALID);
        }
    }
