package com.mickeymeng.admin.infra.service.codegen;
...
import com.mickeymeng.admin.infra.controller.admin.codegen.vo.CodegenCreateListReqVO;
import com.mickeymeng.admin.infra.controller.admin.codegen.vo.table.DatabaseTableRespVO;
import com.mickeymeng.admin.infra.controller.admin.codegen.vo.CodegenUpdateReqVO;
import com.mickeymeng.admin.infra.controller.admin.codegen.vo.table.CodegenTablePageReqVO;
import com.mickeymeng.admin.infra.convert.codegen.CodegenConvert;
import com.mickeymeng.admin.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.mickeymeng.admin.infra.dal.dataobject.codegen.CodegenTableDO;
import com.mickeymeng.admin.infra.dal.mysql.codegen.CodegenColumnMapper;
import com.mickeymeng.admin.infra.dal.mysql.codegen.CodegenTableMapper;
import com.mickeymeng.admin.infra.enums.codegen.CodegenSceneEnum;
import com.mickeymeng.admin.infra.service.codegen.inner.CodegenBuilder;
import com.mickeymeng.admin.infra.service.codegen.inner.CodegenEngine;
import com.mickeymeng.admin.infra.service.db.DatabaseTableService;
...
public class CodegenServiceImpl implements CodegenService {
    ...
    private Long createCodegen0(Long userId, Long dataSourceConfigId, TableInfo tableInfo) {
        ...
        if (codegenTableMapper.selectByTableNameAndDataSourceConfigId(tableInfo.getName(),
            ...
            throw exception(ErrorCodeConstants.CODEGEN_TABLE_EXISTS);
        }
    ...
    private void checkTableInfo(TableInfo tableInfo) {
        if (tableInfo == null) {
            throw exception(ErrorCodeConstants.CODEGEN_IMPORT_TABLE_NULL);
        }
        if (StrUtil.isEmpty(tableInfo.getComment())) {
            throw exception(ErrorCodeConstants.CODEGEN_TABLE_INFO_TABLE_COMMENT_IS_NULL);
        }
        if (CollUtil.isEmpty(tableInfo.getFields())) {
            throw exception(ErrorCodeConstants.CODEGEN_IMPORT_COLUMNS_NULL);
        }
        tableInfo.getFields().forEach(field -> {
            if (StrUtil.isEmpty(field.getComment())) {
                throw exception(ErrorCodeConstants.CODEGEN_TABLE_INFO_COLUMN_COMMENT_IS_NULL, field.getName());
            }
        });
    }
    ...
    public void updateCodegen(CodegenUpdateReqVO updateReqVO) {
        ...
        if (codegenTableMapper.selectById(updateReqVO.getTable().getId()) == null) {
            throw exception(ErrorCodeConstants.CODEGEN_TABLE_NOT_EXISTS);
        }
    ...
    private void syncCodegen0(Long tableId, TableInfo tableInfo) {
        ...
        if (CollUtil.isEmpty(tableFields) && CollUtil.isEmpty(deleteColumnIds)) {
            throw exception(ErrorCodeConstants.CODEGEN_SYNC_NONE_CHANGE);
        }
    ...
    public Map<String, String> generationCodes(Long tableId) {
        ...
        if (CollUtil.isEmpty(columns)) {
            throw exception(ErrorCodeConstants.CODEGEN_COLUMN_NOT_EXISTS);
        }
