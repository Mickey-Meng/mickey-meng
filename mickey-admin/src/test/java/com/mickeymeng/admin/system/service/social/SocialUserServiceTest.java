package com.mickeymeng.admin.system.service.social;
...
import com.mickeymeng.framework.social.core.MickeyAuthRequestFactory;
import com.mickeymeng.framework.test.core.ut.BaseDbAndRedisUnitTest;
import com.mickeymeng.framework.common.service.dto.SocialUserBindReqDTO;
import com.mickeymeng.admin.system.dal.dataobject.social.SocialUserBindDO;
import com.mickeymeng.admin.system.dal.dataobject.social.SocialUserDO;
import com.mickeymeng.admin.system.dal.mysql.social.SocialUserBindMapper;
import com.mickeymeng.admin.system.dal.mysql.social.SocialUserMapper;
...
import static com.mickeymeng.framework.common.core.util.json.JsonUtils.toJsonString;
...
import static com.mickeymeng.framework.common.constants.ErrorCodeConstants.SOCIAL_USER_AUTH_FAILURE;
import static com.mickeymeng.framework.common.constants.ErrorCodeConstants.SOCIAL_USER_NOT_FOUND;
...
public class SocialUserServiceTest extends BaseDbAndRedisUnitTest {
    ...
    @MockBean
    private MickeyAuthRequestFactory authRequestFactory;
    ...
    public void testAuthSocialUser_exists() {
        ...
        String state = "yuanma";
        // mock 方法
        SocialUserDO socialUser = RandomUtils.randomPojo(SocialUserDO.class).setType(type).setCode(code).setState(state);
    ...
    public void testAuthSocialUser_insert() {
        ...
        AuthUser authUser =RandomUtils.randomPojo(AuthUser.class);
    ...
    public void testAuthSocialUser_update() {
        ...
        String state = "yuanma";
        // mock 数据
        socialUserMapper.insert(RandomUtils.randomPojo(SocialUserDO.class).setType(type).setOpenid("test_openid"));
        // mock 方法
        AuthRequest authRequest = mock(AuthRequest.class);
        when(authRequestFactory.get(eq(SocialTypeEnum.GITEE.getSource()))).thenReturn(authRequest);
        AuthUser authUser = RandomUtils.randomPojo(AuthUser.class);
    ...
    public void testGetSocialUserList() {
        ...
        // mock 获得社交用户
        SocialUserDO socialUser = RandomUtils.randomPojo(SocialUserDO.class).setType(SocialTypeEnum.GITEE.getType());
        socialUserMapper.insert(socialUser); // 可被查到
        socialUserMapper.insert(RandomUtils.randomPojo(SocialUserDO.class)); // 不可被查到
        // mock 获得绑定
        socialUserBindMapper.insert(RandomUtils.randomPojo(SocialUserBindDO.class) // 可被查询到
                .setUserId(userId).setUserType(userType).setSocialType(SocialTypeEnum.GITEE.getType())
                .setSocialUserId(socialUser.getId()));
        socialUserBindMapper.insert(RandomUtils.randomPojo(SocialUserBindDO.class) // 不可被查询到
    ...
    public void testBindSocialUser() {
        ...
        // mock 数据：获得社交用户
        SocialUserDO socialUser = RandomUtils.randomPojo(SocialUserDO.class).setType(reqDTO.getType())
                .setCode(reqDTO.getCode()).setState(reqDTO.getState());
        socialUserMapper.insert(socialUser);
        // mock 数据：用户可能之前已经绑定过该社交类型
        socialUserBindMapper.insert(RandomUtils.randomPojo(SocialUserBindDO.class).setUserId(1L).setUserType(UserTypeEnum.ADMIN.getValue())
                .setSocialType(SocialTypeEnum.GITEE.getType()).setSocialUserId(-1L));
        // mock 数据：社交用户可能之前绑定过别的用户
        socialUserBindMapper.insert(RandomUtils.randomPojo(SocialUserBindDO.class).setUserType(UserTypeEnum.ADMIN.getValue())
    ...
    public void testUnbindSocialUser_success() {
        ...
        // mock 数据：社交用户
        SocialUserDO socialUser = RandomUtils.randomPojo(SocialUserDO.class).setType(type).setOpenid(openid);
        socialUserMapper.insert(socialUser);
        // mock 数据：社交绑定关系
        SocialUserBindDO socialUserBind = RandomUtils.randomPojo(SocialUserBindDO.class).setUserType(userType)
    ...
    public void testGetBindUserId() {
        ...
        SocialUserBindDO socialUserBind = RandomUtils.randomPojo(SocialUserBindDO.class).setUserType(userType).setUserId(userId)
