...
import com.mickeymeng.admin.infra.convert.job.JobConvert;
import com.mickeymeng.admin.infra.dal.dataobject.job.JobDO;
import com.mickeymeng.admin.infra.dal.mysql.job.JobMapper;
import com.mickeymeng.admin.infra.enums.job.JobStatusEnum;
import com.mickeymeng.admin.infra.service.job.JobServiceImpl;
...
import com.mickeymeng.framework.middleware.job.core.scheduler.SchedulerManager;
...
public class JobServiceTest extends BaseDbUnitTest {
    ...
    public void testCreateJob_cronExpressionValid() {
        ...
        JobCreateReqVO reqVO = RandomUtils.randomPojo(JobCreateReqVO.class);
        // 调用，并断言异常
        assertServiceException(() -> jobService.createJob(reqVO), ErrorCodeConstants.JOB_CRON_EXPRESSION_VALID);
    }

    @Test
    public void testCreateJob_jobHandlerExists() throws SchedulerException {
        // 准备参数 指定 Cron 表达式
        JobCreateReqVO reqVO = RandomUtils.randomPojo(JobCreateReqVO.class, o -> o.setCronExpression("0 0/1 * * * ? *"));
        // 调用
        jobService.createJob(reqVO);
        // 调用，并断言异常
        assertServiceException(() -> jobService.createJob(reqVO), ErrorCodeConstants.JOB_HANDLER_EXISTS);
    }
    ...
    public void testUpdateJob_jobNotExists(){
        // 准备参数
        JobUpdateReqVO reqVO = RandomUtils.randomPojo(JobUpdateReqVO.class, o -> o.setCronExpression("0 0/1 * * * ? *"));
        // 调用，并断言异常
        assertServiceException(() -> jobService.updateJob(reqVO), ErrorCodeConstants.JOB_NOT_EXISTS);
    }

    @Test
    public void testUpdateJob_onlyNormalStatus(){
        // mock 数据
        JobCreateReqVO createReqVO = RandomUtils.randomPojo(JobCreateReqVO.class, o -> o.setCronExpression("0 0/1 * * * ? *"));
        JobDO job = JobConvert.INSTANCE.convert(createReqVO);
        job.setStatus(JobStatusEnum.INIT.getStatus());
        fillJobMonitorTimeoutEmpty(job);
        jobMapper.insert(job);
        // 准备参数
        JobUpdateReqVO updateReqVO = RandomUtils.randomPojo(JobUpdateReqVO.class, o -> {
            o.setId(job.getId());
            o.setName(createReqVO.getName());
            o.setCronExpression(createReqVO.getCronExpression());
        });
        // 调用，并断言异常
        assertServiceException(() -> jobService.updateJob(updateReqVO), ErrorCodeConstants.JOB_UPDATE_ONLY_NORMAL_STATUS);
    }
    ...
    public void testUpdateJobStatus_changeStatusInvalid() {
        // 调用，并断言异常
        assertServiceException(() -> jobService.updateJobStatus(1L, JobStatusEnum.INIT.getStatus()), ErrorCodeConstants.JOB_CHANGE_STATUS_INVALID);
    }
    ...
    public void testUpdateJobStatus_changeStatusEquals() {
        ...
        jobMapper.insert(job);
        // 调用，并断言异常
        assertServiceException(() -> jobService.updateJobStatus(job.getId(), job.getStatus()), ErrorCodeConstants.JOB_CHANGE_STATUS_EQUALS);
    }
    ...
    public void testGetJobListByIds_success() {
        // mock 数据
        JobDO dbJob = RandomUtils.randomPojo(JobDO.class, o -> {
            o.setStatus(randomEle(JobStatusEnum.values()).getStatus()); // 保证 status 的范围
        });
        JobDO cloneJob = ObjectUtils.cloneIgnoreId(dbJob, o -> o.setHandlerName(RandomUtils.randomString()));
    ...
    public void testGetJobPage_success() {
        ...
        // 测试 handlerName 不匹配
        jobMapper.insert(ObjectUtils.cloneIgnoreId(dbJob, o -> o.setHandlerName(RandomUtils.randomString())));
        // 准备参数
