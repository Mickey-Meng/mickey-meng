package com.mickeymeng.admin.system.controller.admin.user;
...
import com.mickeymeng.admin.system.convert.user.UserConvert;
...
public class UserController {
    ...
    public CommonResult<PageResult<UserPageItemRespVO>> getUserPage(@Valid UserPageReqVO reqVO) {
        ...
        // 获得拼接需要的数据
        Collection<Long> deptIds = CollectionUtils.convertList(pageResult.getList(), AdminUserDO::getDeptId);
    ...
    public void exportUsers(@Validated UserExportReqVO reqVO,
        ...
        // 获得拼接需要的数据
        Collection<Long> deptIds = CollectionUtils.convertList(users, AdminUserDO::getDeptId);
        Map<Long, DeptDO> deptMap = deptService.getDeptMap(deptIds);
        Map<Long, AdminUserDO> deptLeaderUserMap = userService.getUserMap(
                CollectionUtils.convertSet(deptMap.values(), DeptDO::getLeaderUserId));
        // 拼接数据
