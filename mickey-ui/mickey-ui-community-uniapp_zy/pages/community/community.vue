		...
		<u-card title="热门话题" sub-title="查看更多" :border="false" margin="0" :show-foot="false" :head-border-bottom="false"
		 @head-click="moreHotTopic" :full="true">
			<view class="hotTopic" slot="body">
				<u-swiper :list="topicList" mode="rect" :title="true" :effect3d="true" @click="handleTopic"></u-swiper>
			</view>
		</u-card>
	...
	export default {
		...
		methods: {
			...
			getTop4Activies() {
				...
				if(currentCommunity){
					this.$u.get('/interaction/'+currentCommunity.communityId).then(res=>{
						if(res.code==200){
			...
			pubTopic() {
				...
				if(this.currentCommunity.communityId && this.ownerInfo.authenticated){
					this.$u.route('pages/community/postTopic/postTopic');
				}else{
					this.$u.toast('请先认证社区并实名认证')
				}
			},
			//查看帖子明细
