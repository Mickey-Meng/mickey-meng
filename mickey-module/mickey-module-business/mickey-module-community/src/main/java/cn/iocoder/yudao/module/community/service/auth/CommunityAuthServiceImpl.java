package cn.iocoder.yudao.module.community.service.auth;
...
import cn.iocoder.yudao.module.community.service.user.CommunityUserService;
import cn.iocoder.yudao.module.community.controller.app.auth.vo.*;
import cn.iocoder.yudao.module.community.convert.auth.AuthConvert;
...
public class CommunityAuthServiceImpl implements CommunityAuthService {
    ...
    public AppAuthLoginRespVO login(AppAuthLoginReqVO reqVO) {
        // 使用手机 + 密码，进行登录。
        CommunityUserDO user = login0(reqVO.getMobile(), reqVO.getPassword());
    ...
    public AppAuthLoginRespVO smsLogin(AppAuthSmsLoginReqVO reqVO) {
        ...
        // 获得获得注册用户
        CommunityUserDO user = userService.createUserIfAbsent(reqVO.getMobile(), userIp);
    ...
    public AppAuthLoginRespVO socialLogin(AppAuthSocialLoginReqVO reqVO) {
        ...
        // 自动登录
        CommunityUserDO user = userService.getUser(userId);
        if (user == null) {
            throw exception(USER_NOT_EXISTS);
        }

        // 创建 Token 令牌，记录登录日志
        return createTokenAfterLoginSuccess(user, user.getPhoneNumber(), LoginLogTypeEnum.LOGIN_SOCIAL);
    }
    ...
    public AppAuthLoginRespVO weixinMiniAppLogin(AppAuthWeixinMiniAppLoginReqVO reqVO) {
        ...
        // 获得获得注册用户
        CommunityUserDO user = userService.createUserIfAbsent(phoneNumberInfo.getPurePhoneNumber(), getClientIP());
        Assert.notNull(user, "获取用户失败，结果为空");
    ...
    private AppAuthLoginRespVO createTokenAfterLoginSuccess(CommunityUserDO user, String mobile, LoginLogTypeEnum logType) {
        // 插入登陆日志
    ...
    private CommunityUserDO login0(String mobile, String password) {
        final LoginLogTypeEnum logTypeEnum = LoginLogTypeEnum.LOGIN_MOBILE;
        // 校验账号是否存在
        CommunityUserDO user = userService.getUserByMobile(mobile);
        if (user == null) {
    ...
    public void updatePassword(Long userId, AppAuthUpdatePasswordReqVO reqVO) {
        // 检验旧密码
        CommunityUserDO userDO = checkOldPassword(userId, reqVO.getOldPassword());

        // 更新用户密码
        // TODO 芋艿：需要重构到用户模块
        CommunityUserDO communityUserDO = new CommunityUserDO();
        communityUserDO.setId(userDO.getId())
                .setPassword(passwordEncoder.encode(reqVO.getPassword()));
        userMapper.updateById(communityUserDO);
    }

    @Override
    public void resetPassword(AppAuthResetPasswordReqVO reqVO) {
        // 检验用户是否存在
        CommunityUserDO userDO = checkUserIfExists(reqVO.getMobile());
     ...
     */
    @VisibleForTesting
    public CommunityUserDO checkOldPassword(Long id, String oldPassword) {
        CommunityUserDO user = userMapper.selectById(id);
        if (user == null) {
            throw exception(USER_NOT_EXISTS);
        }
        // 参数：未加密密码，编码后的密码
    ...
    public CommunityUserDO checkUserIfExists(String mobile) {
        CommunityUserDO user = userMapper.selectByMobile(mobile);
        if (user == null) {
            throw exception(USER_NOT_EXISTS);
        }
        return user;
